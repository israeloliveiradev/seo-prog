...
export const HealthTemplate: React.FC<TemplateProps> = ({ client, page, pages }) => {
  ...
  const heroData = {
    badge: `Saúde e Bem-estar em ${page?.location || 'sua região'}`,
    title: page?.service_name || "Cuidado e Excelência",
    titleAccent: brand?.company_name || client.name,
    description: brand?.description || page?.meta_description || "Atendimento humanizado e tecnologia de ponta para cuidar do que é mais importante: você.",
    ctaText: "Agendar Consulta",
    ctaLink: `https://wa.me/${brand?.contact_whatsapp}`,
    imageUrl: brand?.hero_image || "https://images.unsplash.com/photo-1576091160550-2173dba999ef?q=80&w=2000"
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-sky-500/30 overflow-x-hidden transition-colors duration-300">
      ...
      {page?.ai_content && (
        <Section className="pt-0">
          <SectionHeader 
            ...
            subtitle="Conheça nossos diferenciais técnicos e nosso compromisso com seu bem-estar."
          />
      ...
      <Section id="features" dark>
        <SectionHeader 
          ...
          subtitle="Infraestrutura completa e equipe dedicada para seu atendimento."
        />
      ...
      <Section id="testimonials">
        <SectionHeader 
          ...
          subtitle="Confiança e resultados que transformam vidas."
        />
      ...
      <footer className="py-20 px-6 border-t border-border text-center space-y-10">
         ...
         <p className="text-[10px] font-black uppercase tracking-[0.5em] text-muted-foreground/20 italic">
           © {new Date().getFullYear()} {client.name} — Health Care Excellence
         </p>
      </footer>
    </div>
  );
