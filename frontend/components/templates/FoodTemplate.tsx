...
export const FoodTemplate: React.FC<TemplateProps> = ({ client, page, pages }) => {
  ...
  const heroData = {
    badge: brand?.company_name || 'Sabor Excepcional',
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12">
            {pages.map((p, i) => (
              <motion.a 
                ...
                className="p-6 bg-muted/30 border border-border rounded-2xl text-center group hover:bg-muted hover:border-orange-500/30 transition-all"
              >
                <p className="text-[10px] font-black uppercase text-muted-foreground mb-2 group-hover:text-orange-500">{p.location}</p>
