...
export const LegalTemplate: React.FC<TemplateProps> = ({ client, page, pages }) => {
  ...
  const fontFamily = brand?.font_family || 'Libre Baskerville';

  const config = defaultLandingConfig;

  const heroData = {
    badge: brand?.company_name || 'Excelência Jurídica',
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12">
            {pages.map((p, i) => (
              <motion.a 
                ...
                className="p-6 bg-muted/30 border border-border rounded-2xl text-center group hover:bg-muted hover:border-slate-500/30 transition-all"
              >
                <p className="text-[10px] font-black uppercase text-muted-foreground mb-2 group-hover:text-slate-500">{p.location}</p>
