...
export const AutomotiveTemplate: React.FC<TemplateProps> = ({ client, page, pages }) => {
  ...
  const fontFamily = brand?.font_family || 'Orbitron';

  const heroData = {
    badge: `Performance em ${page?.location || 'sua região'}`,
    title: page?.service_name || "Velocidade e Estilo",
    titleAccent: brand?.company_name || client.name,
    description: brand?.description || page?.meta_description || "Os melhores veículos e serviços automotivos com garantia de procedência e alta performance.",
    ctaText: "Ver Estoque",
    ctaLink: `https://wa.me/${brand?.contact_whatsapp}`,
    imageUrl: brand?.hero_image || "https://images.unsplash.com/photo-1486262715619-67b85e0b08d3?q=80&w=2000"
  };
    ...
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-red-500/30 overflow-x-hidden transition-colors duration-300">
      ...
      {page?.ai_content && (
        <Section className="pt-0">
          <SectionHeader 
            badge="Potência"
            title="Sua melhor direção"
            subtitle="Qualidade técnica e paixão por motores em cada detalhe do nosso serviço."
          />
      ...
      <Section id="features" dark>
        <SectionHeader 
          badge="Robustez"
          title="Por que acelerar conosco?"
          subtitle="Segurança, garantia e o melhor custo-benefício para o seu veículo."
        />
      ...
      <Section id="testimonials">
        <SectionHeader 
          badge="Satisfação"
          title="Quem acelerou, aprovou"
          subtitle="Relatos de quem encontrou a máquina perfeita em nossa concessionária."
        />
      ...
      <footer className="py-20 px-6 border-t border-border text-center space-y-10">
         ...
         <p className="text-[10px] font-black uppercase tracking-[0.5em] text-muted-foreground/20 italic">
           © {new Date().getFullYear()} {client.name} — Automotive Performance
         </p>
      </footer>
    </div>
  );
