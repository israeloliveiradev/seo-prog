...
export const AutomotiveTemplate: React.FC<TemplateProps> = ({ client, page, pages }) => {
  ...
  const fontFamily = brand?.font_family || 'Oswald';

  const config = defaultLandingConfig;

  const heroData = {
    badge: brand?.company_name || 'Performance Automotiva',
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12">
            {pages.map((p, i) => (
              <motion.a 
                ...
                className="p-6 bg-muted/30 border border-border rounded-2xl text-center group hover:bg-muted hover:border-red-500/30 transition-all"
              >
                <p className="text-[10px] font-black uppercase text-muted-foreground mb-2 group-hover:text-red-500">{p.location}</p>
